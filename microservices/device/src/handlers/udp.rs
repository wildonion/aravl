...
pub async fn process_connection(mut receiver: Receiver::<(Vec<u8>, SocketAddr)>, socket: Arc<UdpSocket>, cass_session: Arc<CassSession>, pg_pool: Pool){

        while let Some((buffer, device_addr)) = //-- keep socket always open to receive all new incoming data from every connected GPS and won't repeat until all async functions in here completes, if there was no loop the socket would be closed after receiving one data from the GPS 
        receiver.recv().await{ //-- receiving buffer and socket
                            ...
                            Ok(data_params) => {
                                ...
                                match GPSData::check_device(data_body).await{
                                    ...
                                    Ok(gps_config) => {
                                        ...
                                        match socket.send_to(config_response.as_str().as_bytes(), device_addr).await{ //-- writing the response into the socket buffer to send back to the GPS - this method will continuously call write until there is no more data to be written
                                            Ok(buffer_bytes) => {
