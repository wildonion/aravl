...
//-- due to no move and no change of the connection between actix threads we didn't put the DbConnection inside Arc<Mutex<T>>. 
pub async fn connection() -> Result<DbConnection, AVL>{
    pool().await.get().map_err(|e| AVL::new(500, format!("⚠️ failed getting db connection: {}", e))) //-- this will return a result
}
